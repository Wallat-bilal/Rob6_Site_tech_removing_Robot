...
ANGLE_OFFSET = -0.12  # 741  # Frame angle offset measured by IMU
...
START_STATE = np.array([[1.44, 0.0, 0.0],
                        [1.20, 0.0, 0.0],
                        [0, 0.0, 0.0]])
FINAL_STATE = np.array([[1.30, 0.0, 0.0],
                        [1.30, 0.0, 0.0],
                        [0, 0.0, 0.0]])

# Tolerance for reaching goal
GOAL_TOLERANCE = np.array([[0.01, 0.001, 0.001],
                           [0.01, 0.001, 0.001],
                           [0.1, 0.001, 0.001]])
...
try:
    ...
    while True:
        ...
        motor_forces_new, f_frame, _, iterations, err, _ = inverse_wire_dynamics(frame_forces_all[1:1+FORCES_SEND_BATCH_SIZE],
                                                                                 wire_dirs[1:1+FORCES_SEND_BATCH_SIZE],
                                                                                 moment_arms[1:1+FORCES_SEND_BATCH_SIZE],
        ...
        # Logging
        log_current_state.append(current_state[:, 0])
        # log_motor_forces.append(motor_forces_new[0])
        # log_wire_vel.append(wire_vel_new[0])

except KeyboardInterrupt as e:
    with open('data.csv', 'w', newline='') as file:
        writer = csv.writer(file)

        # writer.writerow(FINAL_STATE)
        for row in log_current_state:
            writer.writerow(row)
        # for row in log_motor_forces:
        #     writer.writerow(row)
        # for row in log_wire_vel:
        #     writer.writerow(row)
    exit(e)
